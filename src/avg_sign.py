...
class mc_sse_dimer:
    ...
    sign = np.zeros((OP_NUM,4,4), dtype=np.int64) #given an op_num and the initial states, what is the sign
    ...
    def pvect0(self):
        ...
        for iq in range(self.MAX_BOND_NUM):
            ...
            self.wgt[iq] -= self.h * (self.tz_wgt[s1] + self.tz_wgt[s2])/self.Z
    ...
    def vxweight(self):
        ...
        for iq in range(self.MAX_BOND_NUM):
            ...
            if (self.act_tp[ns[0]] != -1 and self.act_tm[ns[1]]!= -1):
                ...
                for i in range(4):
                    self.vx_leg[i,self.nvx] = ns[i]
                matrix_ele = (0.5 * (self.j1 + self.j2) #coeff: 1/2(j1+j2) * 1/2T+T-
                            * 0.5 * self.tp_wgt[ns[0]] * self.tm_wgt[ns[1]])
    ...
    def diagonal_update(self):
        ...
        sign_of_opstring = 1 #initialize as +1 
        ...
        self.avg_sign += sign_of_opstring
    ...
    def measure_sp_heat(self):

        self.num_op_for_sp_heat += self.num_op**2
    ...
    def write_observables(self):
        ...
        sp_heat = self.num_op_for_sp_heat/self.mc_steps - \
            (self.num_op_for_energy/self.mc_steps)**2 - self.num_op_for_energy/self.mc_steps 
        # C = <n^2> - <n>^2 - <n>
        sp_heat /= self.nn
        file = open('sp_heat.txt','a')
        file.write(str(sp_heat)+'\n')
        file.close()
        ...
        sign_val = self.avg_sign/self.mc_steps
        file = open('sign.txt','a')
        file.write(str(sign_val)+'\n')
        file.close()
    ...
    def equilibration(self):
        ...
        for i in range(1,int(self.equil_steps)+1):
            ...
            if anneal is True and i<=0.8*self.equil_steps and i%((0.8*self.equil_steps)//(beta_temp-1)) == 0:
                #simulated annealing, slowly decrease temperature to desired temperature
                self.beta += 1
                with open('log.txt','a') as file:
                    file.write('Running equilibration step {0}, beta is {1}\n'.format(i,self.beta))
            elif anneal is False and i%(self.mc_steps//20) == 0:
                with open('log.txt','a') as file:
